...
colnames(k_train_data)
str(k_train_data)
kosis_train_data2 <- k_train_data %>% select(-전월_대비매출현황) 
kosis_test_data2 <-  k_test_data %>% select(-전월_대비매출현황) 
...
naver_train_data <- train_data %>% select(-전월_대비매출현황, -전월대비_증감)
naver_test_data <- test_data %>% select(-전월_대비매출현황, -전월대비_증감)
...
pred3 <- predict(object = fit3, newdata = naver_test_data, type = 'vector')

RMSE(real, pred3)

#--> 의사결정나무로 모델을 돌리면 훨씬 높은 성능을 보이는데 랜덤포레스트는 오히려 RMSE값이 KOSIS만 돌렸을 때 보다 더 높게 나옴....
#--> 결론 naver 검색추이까지 합쳐서 의사결정나무 돌리는게 훨씬 좋음
#--> 시험끝나면 중감, 감소 컬럼으로 범주형관련 모델생성 

#. 로지스틱, 의사결정나무(범주형)모델 만들기 ---> 시간남으면 
