...
merge_data %>% filter(중분류 == "화장품_미용") %>% 
  ...
  scale_fill_brewer(palette="Pastel1")
#--------------------------증가, 감소 컬럼만들기
#-- 중분류가 아니고 상품군별로 해야겠네... 흐미..
#우선 kosis에 부터 붙이고 그다음 merge_data에!
View(data)
head(kosis_data)
View(kosis_data)
...
data$전월_대비매출현황 <- shift(data$전월_대비매출현황, fill = data$전월_대비매출현황[1])#요거를 계쏙
...
str(merge_data)
str(data)
data$전월대비_증감 <- as.factor(data$전월대비_증감)
data <- as.data.frame(data)

dd <- merge(merge_data, data, 
      by.x = c("년도", "월","판매매체별","중분류"),
      by.y = c("년도", "월","판매매체별","중분류"), all.x = TRUE)

View(dd)
View(dd %>% arrange(년도, 월, 중분류))
#2017- 2에서 1월 빼서 나온 수치만큼 2017-1에서 그 차이를 계산해서 증가인지, 감소인지 확인하기
#kosis_data에 분기넣어야해 
